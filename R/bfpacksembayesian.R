...
BfpackSemBayesianInternal <- function(jaspResults, dataset, options, ...) {
  ...
  ready <- .bfpackOptionsReady(options, type, dataList[["dataset"]])

  # Check if current data allow for analysis
  .bfpackDataReady(dataList[["dataset"]], options, type)

  # Create a container for the results
  bfpackContainer <- .bfpackGetContainer(jaspResults, deps = c("syntax", "model", "seed", "fraction", "standardized", "fixedFactors", "factorStandardisation"))
  ...
  .bfpackSemPathDiagram(dataList[["dataset"]], options, bfpackContainer, ready, jaspResults, position = 5)
}

.bfpackSemGetUsedVars <- function(syntax, availablevars) {
  vv <- availablevars
...
.bfpackSemTranslateModel <- function(syntax, dataset) {
  usedvars <- .bfpackSemGetUsedVars(syntax, colnames(dataset))
...
.bfpackSemPathDiagram <- function(dataset, options, bfpackContainer, ready, jaspResults, position) {
  if (!is.null(bfpackContainer[["pathDiagram"]]) || !options[["pathDiagram"]]) {
    return()
  }
  ...
  if (options[["fixedFactors"]] == "") {
    ...
    bfpackContainer[["pathDiagram"]] <- plot
  } else {
    container <- createJaspContainer(title = gettext("Path Diagram"))
    bfpackContainer[["pathDiagram"]] <- container
  ...
  if (!ready || is.null(bfpackContainer[["lavaanResult"]])) {
    return()
  }
...
.bfpacklavToPlotObj <- function(lavResult) {
