...
# set.seed(100)
# bfpackResult <- bfpack(fit1,hypotheses1,fraction=3,standardize=FALSE)

options <- jaspTools::analysisOptions("BfpackSemBayesian")
...
set.seed(1)
results <- jaspTools::runAnalysis("BfpackSemBayesian", "sesame.csv", options)


test_that("Bayes Factor Matrix table results match", {
	table <- results[["results"]][["bfpackContainer"]][["collection"]][["bfpackContainer_bayesFactorMatrix"]][["data"]]
...
test_that("Bfpack Structural Equation Model table results match", {
...
test_that("Path Diagram plot matches", {
	plotName <- results[["results"]][["bfpackContainer"]][["collection"]][["bfpackContainer_pathDiagram"]][["data"]]
	testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
	jaspTools::expect_equal_plots(testPlot, "path-diagram")
})

test_that("Posterior Probabilities plot matches", {
	plotName <- results[["results"]][["bfpackContainer"]][["collection"]][["bfpackContainer_posteriorProbabilityPlot"]][["data"]]
	testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
...
#     B=~Bb > .6 & B=~Bl > .6 & B=~Bf > .6 & B=~Bn > .6 & B=~Br > .6 & B=~Bc >.6"
# set.seed(100)
# bfpackResult <- bfpack(fit1,hypotheses1,fraction=3,standardize=TRUE)
...
#                 A~B > A~peabody > A~age > 0"
# set.seed(100)
# bfpackResult <- bfpack(fit2, hypotheses2, fraction = 3, standardize = TRUE)
...
#                 A~B > A~peabody > A~age > 0"
# set.seed(100)
# bfpackResult <- bfpack(fit2, hypotheses2, fraction = 3, standardize = FALSE)
...
#    postnumb~prenumb.boy < postnumb~prenumb.girl & postnumb~peabody.boy < postnumb~peabody.girl"
# set.seed(100)
# bfpackResult <- bfpack(fit3, hypotheses3, fraction = 3, standardize = TRUE)
...
#    postnumb~prenumb.boy < postnumb~prenumb.girl & postnumb~peabody.boy < postnumb~peabody.girl"
# set.seed(100)
# bfpackResult <- bfpack(fit3, hypotheses3, fraction = 3, standardize = FALSE)
